...
static void printError(const char* filename, int status, char* endptr, char* source, size_t size) {
	...
	DBJ_GASON_ERR_PRINT("%s:%d:%d: %s\n", filename, lineno + 1, column + 1, dbj_gason_error_str(status));
...
#define TEST_FILE "C:\\Users\\dusan\\source\\repos\\gason\\data\\fathers.json"

	FILE* fp = (argc > 1) ? fopen(argv[1], "r") : fopen(TEST_FILE, "r");

	if (!fp) {
		perror(__FILE__ " can not obtain the file pointer.");
	...
	if (status != JSON_OK) {
		printError((argc > 1 && strcmp(argv[1], "-")) ? argv[1] : "-stdin-", status, endptr, source_, sourceSize);
		goto clear_before_exit;
	}
