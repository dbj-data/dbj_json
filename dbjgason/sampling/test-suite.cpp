///
/// this is left as cpp deliberately
/// - it conforms dbj_gason will work in C++ code
/// - pass(u8R"json(1234567890)json"); can be done in C++ but not in C 
///   thus it might require "cleverness", and cleverness is always finishing in tears
/// 
#include "../gason.h"
#include <string.h>
...
extern "C" void test_suite_parse_(const char*, bool);

static inline void test_suite_parse_(const char* csource, bool ok) {
	char* source = _strdup(csource);
	char* endptr;
	dbj_gason_value value;
	/*dbj_gason_allocator allocator;*/
	int result = dbj_gason_parse(source, &endptr, &value/*, allocator*/);
	if (ok && result) {
		fprintf(stderr, "FAILED %d: %s\n%s\n%*s - \\x%02X\n", parsed, dbj_gason_error_str(result), csource, (int)(endptr - source + 1), "^", *endptr);
		++failed;
	}
	if (!ok && !result) {
...
#define pass(csource) test_suite_parse_(csource, true)
#define fail(csource) test_suite_parse_(csource, false)

extern "C" int test_suite(const int argc, char** argv)
{
	// issue 19 https://github.com/vivkin/gason/issues/19#issue-98596074 
